...
var UserDashboardPaymentView = React.createClass({
  ...
  handleSubmit: function(event) {
    ...
    Stripe.card.createToken(this.state, (status, response)=>{
      ...
      if(status == 200) {
        apiUtils.saveStripeCustomer(response.id, this.props.uid, (error, response)=>{
          if(error) {
...
module.exports = UserDashboardPaymentView;
