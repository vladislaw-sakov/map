...
function updateStreamObjectRating(rating, uid, id) {
  for (var i = 0; i < _activityStream._streamObjects.length; i++) {
    var aStreamObject = _activityStream._streamObjects[i];
...
AppDispatcher.register(function (action) {
  switch (action.actionType) {
    ...
    case SwellStreamConstants.SET_TIP_RATING:
      ...
      SwellStreamStore.emitChange();
      break;
    case SwellStreamConstants.SET_TIP_COMMENT:
      updateStreamObjectRating(action.comment, action.uid, action.id);
      SwellStreamStore.emitChange();
      break;
